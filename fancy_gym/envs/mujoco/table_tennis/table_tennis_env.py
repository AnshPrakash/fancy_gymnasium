...
from fancy_gym.envs.mujoco.table_tennis.table_tennis_utils import (
    is_init_state_valid,
    magnus_force,
)
from fancy_gym.envs.mujoco.table_tennis.table_tennis_utils import (
...
CONTEXT_BOUNDS_4DIMS = np.array([[-1.0, -0.65, -1.0, -0.65], [-0.2, 0.65, -0.2, 0.65]])
CONTEXT_BOUNDS_SWICHING = np.array([[-1.0, -0.65, -1.0, 0.0], [-0.2, 0.65, -0.2, 0.65]])
GOAL_BOUNDS = np.array([[-1.2, -0.6], [-0.2, 0.6]])
...
class TableTennisEnv(MujocoEnv, utils.EzPickle):
        ...
        **kwargs,
        ...
        MujocoEnv.__init__(
            ...
            model_path=os.path.join(
                os.path.dirname(__file__), "assets", "xml", "table_tennis_env.xml"
            ),
    ...
    def _set_ids(self):
        self._floor_contact_id = mujoco.mj_name2id(
            self.model, mujoco.mjtObj.mjOBJ_GEOM, "floor"
        )
        self._ball_contact_id = mujoco.mj_name2id(
            self.model, mujoco.mjtObj.mjOBJ_GEOM, "target_ball_contact"
        )
        self._bat_front_id = mujoco.mj_name2id(
            self.model, mujoco.mjtObj.mjOBJ_GEOM, "bat"
        )
        self._bat_back_id = mujoco.mj_name2id(
            self.model, mujoco.mjtObj.mjOBJ_GEOM, "bat_back"
        )
        self._table_contact_id = mujoco.mj_name2id(
            self.model, mujoco.mjtObj.mjOBJ_GEOM, "table_tennis_table"
        )
        self._id_set = True
    ...
    def step(self, action):
        ...
        for _ in range(self.frame_skip):
            ...
            if not self._terminated:
                if not self._hit_ball:
                    self._hit_ball = self._contact_checker(
                        self._ball_contact_id, self._bat_front_id
                    ) or self._contact_checker(self._ball_contact_id, self._bat_back_id)
                    if not self._hit_ball:
                        ball_land_on_floor_no_hit = self._contact_checker(
                            self._ball_contact_id, self._floor_contact_id
                        )
                        if ball_land_on_floor_no_hit:
                            self._ball_landing_pos = self.data.body(
                                "target_ball"
                            ).xpos.copy()
                            self._terminated = True
                if self._hit_ball and not self._ball_contact_after_hit:
                    if self._contact_checker(
                        self._ball_contact_id, self._floor_contact_id
                    ):  # first check contact with floor
                        self._ball_contact_after_hit = True
                        self._ball_landing_pos = self.data.geom(
                            "target_ball_contact"
                        ).xpos.copy()
                        self._terminated = True
                    elif self._contact_checker(
                        self._ball_contact_id, self._table_contact_id
                    ):  # second check contact with table
                        self._ball_contact_after_hit = True
                        self._ball_landing_pos = self.data.geom(
                            "target_ball_contact"
                        ).xpos.copy()
                        if (
                            self._ball_landing_pos[0] < 0.0
                        ):  # ball lands on the opponent side
        ...
        land_dist_err = (
            np.linalg.norm(self._ball_landing_pos[:-1] - self._goal_pos)
            if self._ball_landing_pos is not None
            else 10.0
        )
    ...
    def _contact_checker(self, id_1, id_2):
        for coni in range(0, self.data.ncon):
            ...
            if (con.geom1 == id_1 and con.geom2 == id_2) or (
                con.geom1 == id_2 and con.geom2 == id_1
            ):
                return True
        return False
    ...
    def get_initial_robot_state(self):
        ...
        robot_init_vel = (
            DEFAULT_ROBOT_INIT_VEL
            + self.np_random.uniform(-1.0, 1.0, size=7) * self._random_vel_scale
        )

        return np.clip(robot_init_pos, jnt_pos_low, jnt_pos_high), np.clip(
            robot_init_vel, jnt_vel_low, jnt_vel_high
        )

    def reset_model(self):
        self._steps = 0
        self._init_ball_state = self._generate_valid_init_ball(
            random_pos=True, random_vel=False
        )
    ...
    def _generate_goal_pos(self, random=True):
        if random:
            return self.np_random.uniform(low=GOAL_BOUNDS[0], high=GOAL_BOUNDS[1])
        else:
    ...
    def _get_obs(self):
        obs = np.concatenate(
            [
                np.cos(self.data.qpos.flat[:7]),
                np.sin(self.data.qpos.flat[:7]),
    ...
    def _get_reward(self, hit_now, land_now):
        ...
        if not self._hit_ball:
            ...
            min_r_b_dist = np.min(
                np.linalg.norm(
                    np.array(self._ball_traj) - np.array(self._racket_traj), axis=1
                )
            )
            return 0.005 * (1 - np.tanh(min_r_b_dist))
        ...
        # Phase 3 hit ball already and not land yet
            ...
            min_b_des_b_dist = np.min(
                np.linalg.norm(
                    np.array(self._ball_traj)[:, :2] - self._goal_pos[:2], axis=1
                )
            )
            return 0.02 * (1 - np.tanh(min_b_des_b_dist))

        # Phase 4 hit ball already and land now
        elif self._hit_ball and land_now:
            over_net_bonus = int(self._ball_landing_pos[0] < 0)
            min_b_des_b_land_dist = np.linalg.norm(
                self._goal_pos[:2] - self._ball_landing_pos[:2]
            )
            return 4 * (1 - np.tanh(min_b_des_b_land_dist)) + over_net_bonus
    ...
    def _generate_random_ball(self, random_pos=False, random_vel=False):
        ...
        x_vel, y_vel, z_vel = 2.5, 0.0, 0.5
        if random_pos:
            x_pos = self.np_random.uniform(
                low=self.context_bounds[0][0], high=self.context_bounds[1][0]
            )
            y_pos = self.np_random.uniform(
                low=self.context_bounds[0][1], high=self.context_bounds[1][1]
            )
        if random_vel:
    ...
    def _generate_valid_init_ball(self, random_pos=False, random_vel=False):
        init_ball_state = self._generate_random_ball(
            random_pos=random_pos, random_vel=random_vel
        )
    ...
    def _get_traj_invalid_penalty(self, action, pos_traj, tau_bound, delay_bound):
        tau_invalid_penalty = 3 * (
            np.max([0, action[0] - tau_bound[1]])
            + np.max([0, tau_bound[0] - action[0]])
        )
        delay_invalid_penalty = 3 * (
            np.max([0, action[1] - delay_bound[1]])
            + np.max([0, delay_bound[0] - action[1]])
        )
    ...
    def get_invalid_traj_step_penalty(self, pos_traj):
        violate_high_bound_error = np.maximum(pos_traj - jnt_pos_high, 0).mean()
        violate_low_bound_error = np.maximum(jnt_pos_low - pos_traj, 0).mean()
        invalid_penalty = violate_high_bound_error + violate_low_bound_error
        return -invalid_penalty

    def get_invalid_traj_step_return(
        self, action, pos_traj, contextual_obs, tau_bound, delay_bound
    ):
        obs = (
            self._get_obs()
            if contextual_obs
            else np.concatenate([self._get_obs(), np.array([0])])
        )  # 0 for invalid traj
        penalty = self._get_traj_invalid_penalty(
            action, pos_traj, tau_bound, delay_bound
        )
        return (
            obs,
            penalty,
    ...
    def check_traj_validity(action, pos_traj, vel_traj, tau_bound, delay_bound):
        time_invalid = (
            action[0] > tau_bound[1]
            or action[0] < tau_bound[0]
            or action[1] > delay_bound[1]
            or action[1] < delay_bound[0]
        )
        if (
            time_invalid
            or np.any(pos_traj > jnt_pos_high)
            or np.any(pos_traj < jnt_pos_low)
        ):
...
class TableTennisGoalSwitching(TableTennisEnv):
    def __init__(self, frame_skip: int = 4, goal_switching_step: int = 99, **kwargs):
        super().__init__(
            frame_skip=frame_skip, goal_switching_step=goal_switching_step, **kwargs
        )
