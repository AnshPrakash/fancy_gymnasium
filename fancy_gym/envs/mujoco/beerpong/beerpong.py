...
class BeerPongEnv(MujocoEnv, utils.EzPickle):
    ...
    def __init__(self, **kwargs):
        ...
        self.model = None
        self.geom_id = lambda x: mujoco.mj_name2id(
            self.model, mujoco.mjtObj.mjOBJ_GEOM, x
        )
    ...
    def step(self, a):
        ...
        for _ in range(self.repeat_action):
            applied_action = (
                a
                + self.data.qfrc_bias[: len(a)].copy() / self.model.actuator_gear[:, 0]
            )
            try:
        ...
        if not crash:
            ...
            is_collided = reward_infos[
                "is_collided"
            ]  # TODO: Remove if self collision does not make a difference
        ...
        infos = dict(
            ...
            q_vel=self.data.qvel[0:7].ravel().copy(),
            sim_crash=crash,
        )
    ...
    def _get_obs(self):
        ...
        cup_goal_diff_final = (
            ball_pos - self.data.site("cup_goal_final_table").xpos.copy()
        )
    ...
    def _get_reward(self, action):
        ...
        self.dist_ground_cup = (
            np.linalg.norm(ball_pos - goal_pos)
            if self.ball_ground_contact_first and self.dist_ground_cup == -1
            else self.dist_ground_cup
        )
        action_cost = np.sum(np.square(action)) / 7
        ...
        if self._steps == MAX_EPISODE_STEPS_BEERPONG - 1:  # or self._is_collided:
            ...
            if self.ball_ground_contact_first:
                (
                    min_dist_coeff,
                    final_dist_coeff,
                    ground_contact_dist_coeff,
                    rew_offset,
                ) = 1, 0.5, 2, -4
            else:
                if not self.ball_in_cup:
                    if (
                        not self.ball_table_contact
                        and not self.ball_cup_contact
                        and not self.ball_wall_contact
                    ):
                        (
                            min_dist_coeff,
                            final_dist_coeff,
            ...
            action_cost = np.mean(self.action_costs)
            reward = (
                rew_offset
                - min_dist_coeff * min_dist**2
                - final_dist_coeff * final_dist**2
                - action_cost
                - ground_contact_dist_coeff * self.dist_ground_cup**2
            )
            # release step punishment
            min_time_bound = 0.1
            max_time_bound = 1.0
            release_time = self.release_step * self.dt
            release_time_rew = int(release_time < min_time_bound) * (
                -30 - 10 * (release_time - min_time_bound) ** 2
            ) + int(release_time > max_time_bound) * (
                -30 - 10 * (release_time - max_time_bound) ** 2
            )
        ...
        infos = {
            ...
            "is_collided": False,
        }  # TODO: Check if is collided is needed
        return reward, infos

    def _check_contacts(self):
        # TODO proper contact detection using sensors?
        if not self.ball_table_contact:
            self.ball_table_contact = self._check_collision(
                {self.geom_id("ball_geom")}, {self.geom_id("table_contact_geom")}
            )
        if not self.ball_cup_contact:
            self.ball_cup_contact = self._check_collision(
                {self.geom_id("ball_geom")},
                {self.geom_id(name) for name in CUP_COLLISION_OBJ},
            )
        if not self.ball_wall_contact:
            self.ball_wall_contact = self._check_collision(
                {self.geom_id("ball_geom")}, {self.geom_id("wall")}
            )
        if not self.ball_in_cup:
            self.ball_in_cup = self._check_collision(
                {self.geom_id("ball_geom")}, {self.geom_id("cup_base_table_contact")}
            )
        if not self.ball_ground_contact_first:
            if (
                ...
                self.ball_ground_contact_first = self._check_collision(
                    {self.geom_id("ball_geom")}, {self.geom_id("ground")}
                )
    ...
    def step(self, a):
        if self._steps < FIXED_RELEASE_STEP:
            ...
            while self._steps < MAX_EPISODE_STEPS_BEERPONG:
                obs, sub_reward, terminated, truncated, infos = super(
                    BeerPongEnvStepBasedEpisodicReward, self
